...
use super::utf8::{decode_last_utf8, decode_utf8};
...
impl<'t> Input for CharInput<'t> {
    ...
    fn succ(&self, at: InputAt) -> Char {
        at.char()
    }

    fn pred(&self, at: InputAt) -> Char {
        decode_last_utf8(&self[..at.pos()]).map(|(c, _)| c).into()
    }

    fn is_empty_match(&self, at: InputAt, empty: &InstZero) -> bool {
        match empty.look {
            Zero::StartLine => {
                let c = self.pred(at);
                at.pos() == 0 || c == '\n'
            ...
            Zero::StartText => at.pos() == 0,
            Zero::EndText => at.pos() == self.len(),
            Zero::WordBoundary => {
                let (c1, c2) = (self.pred(at), self.succ(at));
