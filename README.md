...
| `a*` (kleen star) | `!a` (exponential conjunction/of course),<br/>□ (necessity) | `Exp(a)` | μ, fixed point/trace |
| `a*?` (non greedy) | `?a` (exponential disjunction/why not),<br/>◊ (possibility) | `Exp(a)` | ν |
| `a?` () | `a` | `Or(Zero, a)` | |
